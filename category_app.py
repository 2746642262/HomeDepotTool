...
class CategoryApp(QMainWindow):
    ...
    def populate_tree_from_xml(self, xml_node, parent):
        for child in xml_node:
            ...
            if t:
                item = QTreeWidgetItem(parent); item.setText(0, t); item.setData(0, Qt.ItemDataRole.UserRole, t); item.setFlags(item.flags() | Qt.ItemFlag.ItemIsEditable); self.set_favorite_state(item, False)
    ...
    def save_project(self):
        if not self.current_project_path:
            path, _ = QFileDialog.getSaveFileName(self, "Save", "Team.json", "JSON (*.json)")
            if not path: return
            self.current_project_path = path
        data = self.serialize_tree(self.tree_widget.invisibleRootItem())
